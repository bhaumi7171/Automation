...
public class Draganddrop extends Base{
	...
	{
		driver.navigate().to("https://selenium.qabible.in/drag-drop.php");//otherstab
		WebElement drag=driver.findElement(By.xpath("//span[text()='Draggable n°1']"));
		Actions action=new Actions(driver);
		action.moveToElement(drag);
		action.click(drag).perform();//doubleClick(drag).perform();
		...
		action.dragAndDrop(drag1, drop1).build().perform();// build : method that is called if there are multiple actions in a script 
